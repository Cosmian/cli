...
fn run_configure_wizard(mut config: ClientConfig) -> CosmianResult<()> {
    ...
    // Findex (optional)
    let configure_findex: bool = Confirm::new()
        .with_prompt("Configure Findex server settings as well?")
        .default(config.findex_config.is_some())
        .interact()
        .map_err(|e| cli_error!("Prompt failed: {e}"))?;
    if configure_findex {
        if let Some(findex_cfg) = config.findex_config.as_mut() {
        ...
        config.findex_config = None;
    }

    // Save to default path explicitly (ignore env override to satisfy requirement)
    let default_path = get_default_conf_path(crate::config::COSMIAN_CLI_CONF_PATH)
